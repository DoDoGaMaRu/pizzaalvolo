import "./PizzaTable.css"
import {Link, useNavigate} from "react-router-dom";
export default function PizzaTable({pizza_list, cur_page}) {
    const navigate = useNavigate()
    const move1 = () => {
        const id = pizza_list[2 * (cur_page - 1)].id
        navigate(`/review/${id}`, {state: {...pizza_list[2 * (cur_page - 1)]}})
    }
    const move2 = () => {
        const id = pizza_list[2 * (cur_page - 1) + 1].id
        navigate(`/review/${id}`, {state: {...pizza_list[2 * (cur_page - 1) + 1]}})
    }
    return (
        <div className="pizza-card-form">
            <div className="pizza-menu-card">
                <div className="pizza-menu-item">
                    <div className="container">
                        <div className="pizza-img-container">
                            <img src={pizza_list[2 * (cur_page - 1)].img_url} alt="" height="240px"/>
                        </div>
                        <div className="item">
                            <div>
                                {pizza_list[2 * (cur_page - 1)].name}
                            </div>
                            <div>
                                {pizza_list[2 * (cur_page - 1)].price}
                            </div>
                        </div>
                    </div>
                    <div className="click-active-shadow-box">
                        <div className="view-button" onClick={() => move1()}>
                            <div className="inner-container">
                                <img src="data:image/png;base64,"
                                    className="view-card" /> 리뷰보기
                            </div>
                        </div>
                        <div className="basket-button" onClick={(e) => AddBasket(pizza_list[2 * (cur_page - 1)].id)}>
                            <div className="inner-container">
                                <img src="data:image/png;base64,"
                                     className="basket-card" /> 장바구니
                            </div>
                        </div>
                        </div>
                </div>
            </div>
            {
                pizza_list.length - 1 === 2 * (cur_page - 1) ?
                    null :
                    <div className="pizza-menu-card">
                        <div className="pizza-menu-item">
                            <div className="container">
                                <div className="pizza-img-container">
                                    <img src={pizza_list[2 * (cur_page - 1) + 1].img_url} alt="" height="240px"/>
                                </div>
                                <div className="item">
                                    <div>
                                        {pizza_list[2 * (cur_page - 1) + 1].name}
                                    </div>
                                    <div>
                                        {pizza_list[2 * (cur_page - 1) + 1].price}
                                    </div>
                                </div>
                            </div>
                            <div className="click-active-shadow-box">
                                <div onClick={() => move2()} className="view-button">
                                    <div className="inner-container">
                                        <img src="data:image/png;base64,"
                                             className="view-card" /> 리뷰보기
                                    </div>
                                </div>
                                <div className="basket-button" onClick={(e) => AddBasket(pizza_list[2 * (cur_page - 1) + 1].id)}>
                                    <div className="inner-container">
                                        <img src="data:image/png;base64,"
                                             className="basket-card" /> 장바구니
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
            }
        </div>
    );
}

function AddBasket(id) {
    alert('장바구니에 담겼습니다.')
    const post = {user_sn: 1, menu_id: id, count: 1}
    fetch('http://202.31.202.34:443/basket/insert', {
        method: "post",
        headers: {
            "content-type": "application/json",
        },
        body: JSON.stringify(post),
    })
}